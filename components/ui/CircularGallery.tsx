...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = "#545050", font = "30px sans-serif" }: TitleArgs) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class Media {
  ...
  onResize(resizeObj?: { screen?: { width: number; height: number }; viewport?: { width: number; height: number } }) {
    if (resizeObj?.screen) this.screen = resizeObj.screen;
    if (resizeObj?.viewport) this.viewport = resizeObj.viewport;
